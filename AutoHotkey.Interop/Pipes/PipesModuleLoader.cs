...
namespace Kalih.Interop.Pipes
{
    public static class PipesModuleLoader
    {
        ...
        private static void InitPipeClient(string pipename) {
            ...
            if (!AEngine.Instance.FunctionExists("pipeclient_getversion")) {
                var ahk_pipeclient_lib = Util.EmbededResourceHelper.ExtractToText(typeof(PipesModuleLoader).Assembly, "Pipes/pipeclient.ahk");
                AEngine.Instance.LoadScript(ahk_pipeclient_lib);
            }
            else {
                AEngine.Instance.LoadScript("A__PIPECLIENT.close()");
            }

            AEngine.Instance.LoadScript(string.Format(
                    "A__PIPECLIENT := new pipeclient({0})",
                    AEscape.Quote(pipename)));
        }
